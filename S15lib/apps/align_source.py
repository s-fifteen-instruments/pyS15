...
def show_source_properties(dev_path: str = None, logging: bool = True, 
    ...
    while True:
        try:
            ...
            print(
                'Pair rate corrected for accidentals (1/s): {:.0f}'.format(pair_rate))
            print('Avg. efficiency: {:.2f}%'.format(efficiency * 100))
            print(
                'Singles channel1 (1/s): {:.0f}\tSingles channel2 (1/s): {:.0f}'.format(rate_ch1, rate_ch2))
            print(
                'pairs / acc: {:.2f}'.format((pair_rate / (2 * acc_rate_per_bin))))
...
if __name__ == '__main__':
    show_source_properties(
        dev_path='/dev/tty.usbmodemTDC1_00141', logging=False)
